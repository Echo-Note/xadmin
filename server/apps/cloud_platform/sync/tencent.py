...
class TencentCloudSyncer(BaseCloudSyncer):
    ...
    def _build_client(self, client_cls, region='ap-guangzhou', endpoint=''):  # noqa: ANN001, ANN202
        """构建腾讯云 API 客户端。

        Args:
            client_cls: 客户端类（如 CvmClient）。
            region: 区域标识。
            endpoint: API 端点域名（如 domain.tencentcloudapi.com）。
    ...
    @staticmethod
    def _safe_get(obj, attr: str, default=''):  # noqa: ANN001
    ...
    def _fetch_info_templates(self) -> dict[str, dict]:
        ...
        通过 DescribeTemplateList 拉取所有模板，提取 ContactInfo 的企业名称/
        联系人信息和 CertificateInfo 的统一社会信用代码。
        ...
        try:
            client = self._build_client(domain_client.DomainClient, endpoint='domain.tencentcloudapi.com')
            offset = 0
            while True:
                ...
                for tmpl in resp.TemplateSet:
                    ...
                    if cert_info:
                        ...
                        # TYDMZ：统一社会信用代码证书；QTTYDM：其他-统一社会代码证书
                        # YYZZ：三证合一后营业执照号即统一社会信用代码
    ...
    def _fetch_domains(self) -> list[DomainSyncData]:
        ...
        执行顺序：
        1. 拉取信息模板列表 → 缓存 {TemplateId: 模板详情}
        2. 拉取域名名称列表
        3. 逐域名获取详情，通过 TemplateId 关联模板中的企业名称和信用代码
        """
        if not self._setup():
            return []
            ...
            try:
                ...
                if info.ExpirationDate:
                    try:
                        ...
                        pass
                # DomainStatus 可能是 list 或 string
                ...
                # 主体类型：优先模板中的 RegistrantType，其次 DomainInfo
    ...
    def _is_platform_dns(self, domain) -> bool:  # noqa: ANN001
        ...
        免费版：onion.dnspod.net / parameter.dnspod.net
        专业版：ns3.dnsv2.com / ns4.dnsv2.com
        ...
        return 'dnspod' in dns or 'dnsv2' in dns
    ...
    def _fetch_dns_records(self) -> list[DnsRecordSyncData]:
        ...
        try:
            client = self._build_client(dnspod_client.DnspodClient, endpoint='dnspod.tencentcloudapi.com')
    ...
    def _fetch_balance(self) -> BalanceSyncData | None:
        ...
        if not self._setup():
            return None
        try:
            client = self._build_client(billing_client.BillingClient, endpoint='billing.tencentcloudapi.com')
